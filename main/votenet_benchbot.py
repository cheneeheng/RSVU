...
# install tools) 
ROOT_DIR = '/benchbot_submission/votenet'
...
def compute_world_T_camera(pose_rotation, pose_translation):
    """
    This function computes the transformation matrix T that transforms one point
    in the camera coordinate system to the world coordinate system. This
    function needs the scipy package - scipy.spatial.transform.Rotation
    P_w = P_r * T, where P_r = [x, y, z] is a row vector in the camera
    coordinate (x - right, y - down, z - forward) and P_w the location of the
    point in the world coordinate system (x - right, y - forward, z - up)

    :param pose_rotation: [quat_x, quat_y, quat_z, quat_w], a quaternion that
                          represents a rotation 
    :param pose_translation: [tr_x, tr_y, tr_z], the
                             location of the robot origin in the world coordinate
                             system 
...
def votenet_detection(net, observations):
    ...
    if detections[0]:
        ...
        for detection in detections:
            ...
            # convert the 3D box corners in the camera coordinate system to
            ...
            result["box_corners_camera"] = detection[1]
            box_center_camera = np.sum(detection[1], axis=0) / 8.0
            result["centroid_camera"] = box_center_camera
            box_extent_camera = np.sum(
                np.abs(detection[1] - box_center_camera), axis=0) / 4.0
            result["extent_camera"] = box_extent_camera
